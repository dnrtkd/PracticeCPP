...
void MonsterDataInput(int index, const char* name, int level, int hp, int att, int def, int quick, int exp)
{
...
int main(void)
{
	...
	MonsterDataInput(0, "꼬꼬닭", 1, 30, 5, 5, 5, 30);
	MonsterDataInput(1, "상경쥐", 2, 35, 6, 5, 5, 40);
	MonsterDataInput(2, "구리왕", 3, 60, 7, 7, 5, 60);
	MonsterDataInput(3, "철퇴병사", 5, 100, 10, 10, 5, 80);
	MonsterDataInput(4, "남한궁병", 6, 70, 12, 5, 9, 90);
	MonsterDataInput(5, "도부수", 7, 150, 12, 8, 6, 150);
	MonsterDataInput(6, "조총수", 10, 110, 15, 5, 7, 120);
	MonsterDataInput(7, "칼잡이", 11, 130, 13, 8, 6, 130);
	MonsterDataInput(8, "사무라이", 12, 250, 16, 12, 10, 300);
		...
		if (i == 1)
		{
			if (player.status.quick >= p->quick)
			{
				if (player.status.att > p->def)
					monsCurrHp -= player.status.att - p->def;
				else
					monsCurrHp -= 1;

				if (player.status.def < p->att)
					PlayCurrHp -= player.status.att - p->def;
				else
					PlayCurrHp -= 1;
			}
			else
